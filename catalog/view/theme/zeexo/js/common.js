...
$(document).ready(function() {
	...
	// Load cart count
	$('.open-cart .cart-count').load('indexe061.html?route=common/cart/info#total_count_ajax');
		...
		$.ajax({
			...
			success: function(json) {			
				...
				if (json['success']) {
					...
					$('#modalAddToCartProduct .tt-cart-total .tt-price').html(' ').load('indexe061.html?route=common/cart/info#total_amount_ajax');
					...
					$('#cart_block #total_count_ajax, .open-cart .cart-count').load('indexe061.html?route=common/cart/info#total_count_ajax');
				}
			}
		});
	},
